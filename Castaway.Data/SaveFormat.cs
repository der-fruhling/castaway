...
namespace Castaway.Data
{
    ...
    public abstract class SaveFormat
    {
        ...
        public async Task Save(string path) => await Task.Run(async () =>
        {
            ...
            foreach (var f in toSave)
            {
                ...
                logger.Verbose("Wrote data ({Size} bytes) for unit {Unit}({Type}) at {Index} (actually {ActualPos})", 
        ...
        public async Task Load(string path) => await Task.Run(async () =>
        {
            var logger = CastawayGlobal.GetLogger(typeof(SaveFormat));
            ...
            foreach (var (f, tup) in d)
            {
                if (tup == null)
                {
                    try
                    {
                        ...
                        logger.Warning("No value for unit {Unit} from save file {Path} : {$This}, set to null", f.Name, path, this);
                    }
